...
class TimeRecordsService {
    ...
    getAll(): Promise<any> {
        return client.get(`/records/${this.stations}`);
    }

    getWithStartDate(date: Date): Promise<any> {
        return client.get(`/records/${this.stations}?startDate=${date}`);
    }

    getWithEndDate(date: Date): Promise<any> {
        return client.get(`/records/${this.stations}?endDate=${date}`);
    }

    getWithStartAndEndDate(startDate: Date, endDate: Date): Promise<any> {
        return client.get(`/records/${this.stations}?startDate=${startDate}&endDate=${endDate}`);
    }
